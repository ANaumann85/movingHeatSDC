...
\begin{algorithm2e}
	\caption{Multi-rate SDC prediction step.}
        \SetKwInOut{Input}{input}
         ...
         \Input{$u_0$}
         \Output{$f(u^0_m)$ and $g(u^0_{m,p})$ for $m=1,\ldots,M$ and $P=1,\ldots,P$.}
         $u^0_0 \leftarrow u_0$\\
         \For{$m=1, M$}{
...
%
\begin{algorithm2e}
	\caption{Multi-rate SDC prediction sweep.\todo{initial value for first embedded step is not correct... explicit terms cancel out}}
        \SetKwInOut{Input}{input}
         \SetKwInOut{Output}{output}
         \Input{$u_0$ and $f(u^k_m)$, $g(u^k_{m,p})$ for $m=1,\ldots,M$ and $p=1,\ldots,P$.}
         ...
         \For{$m=1,M$}{
         	Solve $u^* = u^{k+1}_{m-1} + \Delta t_m \left( f(u^{*}_{m}) - f(u^k_{m}) \right) + I_{m-1}^{m}$ \\
		$f^* \leftarrow f(u^*)$\\
		$u^{k+1}_{m-1,0} \leftarrow u^{k+1}_{m-1}$\\
		\For{$p=1,P$}{
			$u_{m-1,p}^{k+1} = u_{m-1,p-1}^{k+1} + \Delta t_{m,p} \left( f(u^{*}) - f(u^k_{m}) \right) + \Delta t_{m,p} \left( g(u^{k+1}_{m-1,p-1}) - g(u^k_{m-1,p-1}) \right)  + I_{m,p-1}^{p}$\\
