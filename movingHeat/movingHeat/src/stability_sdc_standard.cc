...
int main(int argc, char* argv[])
{
  ...
  typedef Sdc<Heat::VectorType, 3> Method;
  Method::Init init([&heat](Heat::VectorType& d) { heat.init(d); d=0.0; });
  Method sdc(init, 2, "radau_right");
  Heat::VectorType y0;
  heat.init(y0); y0 = 0.0;

  unsigned nTests(20);
  auto alpha_vec=linspace(0.0, 5.0, nTests);
  auto nu_vec=linspace(0.0, 10.0, nTests);
...
#else
  std::fstream eigOut("stabEigs_sdc_standard.mtx", std::ios_base::out | std::ios_base::trunc);
  eigOut << maxEigs;
  eigOut.close();
#endif
