...
template<int M>
void solve(Heat& heat, Heat::VectorType& y0, unsigned k_iter, unsigned nStep, unsigned P)
{
  switch(P) 
  {
    case 1:
      solve<M,1>(heat, y0, k_iter, nStep);
      break;
    case 2:
      solve<M,2>(heat, y0, k_iter, nStep);
      break;
    case 3:
      solve<M,3>(heat, y0, k_iter, nStep);
      break;
    case 4:
      solve<M,4>(heat, y0, k_iter, nStep);
      break;
    case 5:
      solve<M,5>(heat, y0, k_iter, nStep);
      break;
    case 6:
      solve<M,6>(heat, y0, k_iter, nStep);
      break;
    case 7:
      solve<M,7>(heat, y0, k_iter, nStep);
      break;
    case 8:
      solve<M,8>(heat, y0, k_iter, nStep);
      break;
    default:
...
void solve(Heat& heat, Heat::VectorType& y0, unsigned k_iter, unsigned nStep, unsigned M, unsigned P)
{
  switch(M) {
    case 1:
      solve<1>(heat, y0, k_iter, nStep, P);
      break;
    case 2:
      solve<2>(heat, y0, k_iter, nStep, P);
      break;
    case 3:
      solve<3>(heat, y0, k_iter, nStep, P);
      break;
    case 4:
      solve<4>(heat, y0, k_iter, nStep, P);
      break;
    case 5:
      solve<5>(heat, y0, k_iter, nStep, P);
      break;
    case 6:
      solve<6>(heat, y0, k_iter, nStep, P);
      break;
    case 7:
      solve<7>(heat, y0, k_iter, nStep, P);
      break;
    case 8:
      solve<8>(heat, y0, k_iter, nStep, P);
      break;
  }
}
...
int main(int argc, char* argv[])
{
  ...
  Heat heat(20, nu, alpha, v0, src, useLapl0, addConstRobin, laplTilde);
  Heat::VectorType y0;
  heat.init(y0); 
  if(addConstRobin) {
    y0 = heat.getBVal();
