...
struct MRSdc
{
  ...
  void predict(F& f, const Vec& u0, double t0, double te, bool setInter=true)
  {
    ...
    //Vec swap; swap = u0;
    Vec fastFVal; fastFVal = u0;
    Vec slowExplVal; slowExplVal = u0;
    f.fast(coll.coll_sub[0].tleft, u0_step, fastFVal);
    f.slowExpl(coll.coll_sub[0].tleft, u0_step, slowExplVal);
    ...
    for(unsigned m(0); m < M; ++m)
    {
      ...
      //f.fast(coll.coll_sub[m].tleft, u0_step, fVal);//fVal=g(t_m, u^0_m) //use fast val from line 136 and line 169
      axpy(coll.coll_sub[m].delta_m[0], fastFVal, rhs);//rhs=Mu^0_{m,0}+dt_{m,0}*g(t_m,u^0_m)
      f.slowImpl(coll.coll.nodes[m], us[m], fVal); //fVal=f^I(t_m, u^*_m)
      axpy(1.0, slowExplVal, fVal); //fVal = f^I(t_m, u^*_m)+f^E(t_{m-1}, u_{m-1})
      axpy(coll.coll_sub[m].delta_m[0], fVal, rhs);//rhs=Mu^0_{m,0}+dt_{m,0}*g(t_m,u^0_m)+dt_{m,0}(f^I(t_m,u^*_m)+f^E(t_{m-1}, u_{m-1})
      ...
      f.slowExpl(coll.coll.nodes[m], us[m], slowExplVal);
    }
  }
  ...
  void sweep(F& f, Vec& u0, double t0, double te, bool setInter=true)
  {
    ...
    Vec u0_step; u0_step = u0;
    Vec fuStar, fuPm1Old, slowExplVal;
    init(fuStar); init(fuPm1Old); init(slowExplVal);
    Vec fusI, fEdiff;
    init(fusI); init(fEdiff);
    for(unsigned m(0); m < M; ++m) {
      //standard step for u*
      f.Mv(u0_step, rhs); //rhs=Mu^{k+1}_{m}
      f.slowImpl(coll.coll.nodes[m], us[m], fVal);
      axpy(-coll.coll.delta_m[m], fVal, rhs);//rhs=Mu^{k+1}_{m}-dt_{m}(f^I(t_m,u^k_m))
      if(m > 0) {
        axpy(-coll.coll.delta_m[m], slowExplVal, rhs);
        f.slowExpl(coll.coll.nodes[m-1], us[m-1], fEdiff);
        axpy(coll.coll.delta_m[m], fEdiff, rhs); //rhs=Mu^{k+1}_{m}-dt_{m}(f^I(t_m,u^k_m)+f^E(t_m, u^k_m))
        axpy(-1.0, slowExplVal, fEdiff);
      }
      f.slowExpl(coll.coll.nodes[m], us[m], slowExplVal);
      ...
      f.slowImpl(coll.coll.nodes[m], fuPm1Old, fuStar); //fuStar=f^I(t_m, u^*_m)
      f.slowImpl(coll.coll.nodes[m], us[m], fusI);
      axpy(-1.0, fusI, fuStar);
      if(m > 0)
        axpy(1.0, fEdiff, fuStar);
      //us[m] = fuPm1Old;

      //embedded steps
      f.Mv(u0_step, rhs); //rhs=M u^{k+1}_{m}
      axpy(thetaSlow*coll.coll_sub[m].delta_m[0], fuStar, rhs); //rhs=M u^{k+1}_{m}+dt_{m,0}f(t_m, u^*_m)
      //axpy(-thetaSlow*coll.coll_sub[m].delta_m[0], fus[m], rhs); //rhs = Mu^{k+1}_m+dt_{m,0}f(t_m, u^*_m)-dt_{m,0}f(t_m, u^k_m)
      ...
      for(unsigned p(1); p < P; ++p) {
        ...
        axpy(thetaSlow*coll.coll_sub[m].delta_m[p], fuStar, rhs); //rhs=M u^{k+1}_{m,p-1}+dt_{m,p}f(t_m, u^*_m)
        //axpy(-thetaSlow*coll.coll_sub[m].delta_m[p], fus[m], rhs); //rhs=M u^{k+1}_{m,p-1}+dt_{m,p}f(t_m, u^*_m)-dt_{m,p}f(t_m,u^k_m)
